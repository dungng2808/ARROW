...
```text
python -m preflight.cli --config ../Java-version/config.local.toml --input-root ../classes2test --shard ../shards-5/shard-02.json --workers 5 --output-dir runs/kieu_anh-shard-02-<RUN_ID>
```

Nếu cần dừng, nhấn `Ctrl+C` một lần và chờ tool thoát mã `130`. Session sau tiếp tục bằng:

```text
python -m preflight.cli --resume --output-dir runs/kieu_anh-shard-02-<RUN_ID> --workers 5
```
