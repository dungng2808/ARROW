...
| `JDK_UNSUPPORTED` | `reason_codes`, `java_version`, command và log build | `JDK_HOME_MISSING`, `JDK_JAVA_MISSING`, `JDK_JAVAC_MISSING`, sai major/path, Maven/Gradle dùng JDK khác; với Gradle `1.8` hoặc `VERSION_1_8`, xác minh parser trả `8` chứ không phải `1` | Target Java 6/7 nằm ngoài ma trận nhóm: ghi nhận, không tự cài thêm hoặc map giả sang JDK 17 |
...
1. **JDK:** với mỗi `JDK_UNSUPPORTED`, tách `JDK_VERSION_MISMATCH` khỏi lỗi
   ...
   ngoài ma trận (bao gồm 6/7), ghi rõ target và giữ `JDK_UNSUPPORTED`;
   **không tự tìm, tải, cài hoặc thêm mapping** JDK ngoài ma trận trong task
   này. Nếu parser nhận sai target, báo bug tool, không cài JDK theo target
   sai hoặc sửa code khi run đang chạy.
