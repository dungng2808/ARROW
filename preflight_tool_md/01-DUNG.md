# Giao việc preflight — Dũng — shard 01
...
- Có `shards-5/shard-01.json`, `shards-5/summary.json`, `shards-5/README.md`, `preflight.md`, `preflight_tool/TEST_REPORT_FIX_FOUR_20260923.md`, `Java-version/AGENTS.md`. File phân công parse được và hash khớp summary.
...
Agent hãy đọc **toàn bộ file này**, các AGENTS.md áp dụng và tài liệu được dẫn dưới đây, rồi thực hiện trên máy của **Dũng**. Không chỉ đưa lại hướng dẫn hoặc dừng ở index-only: mục tiêu là chạy hết phần được giao, kiểm tra output và viết báo cáo bàn giao nếu môi trường đáp ứng điều kiện an toàn.

- Repository: ARROW trên máy hiện tại; tự tìm đường dẫn, không hard-code username/ổ đĩa của người khác.
- File phân công duy nhất: `shards-5/shard-01.json`.
...
3. Xác nhận CLI có `--shard`. Kiểm SHA-256 file `shard-01.json` với entry tương ứng trong `shards-5/summary.json`; số class phải đúng 17164. Nếu thiếu/mismatch thì dừng trước build và báo lỗi.
...
```text
python -m preflight.cli --config ../Java-version/config.local.toml --input-root ../classes2test --shard ../shards-5/shard-01.json --index-only --output-dir runs/dung-shard-01-<RUN_ID>-check
```

Phải kiểm tra:

- Exit code 0 và `provenance.json` có `shard_id = shard-01`, `classes_selected = 17164`.
...
```text
python -m preflight.cli --config ../Java-version/config.local.toml --input-root ../classes2test --shard ../shards-5/shard-01.json --workers 5 --output-dir runs/dung-shard-01-<RUN_ID>
```

Nếu cần dừng, nhấn `Ctrl+C` một lần và chờ tool thoát mã `130`. Session sau tiếp tục bằng:

```text
python -m preflight.cli --resume --output-dir runs/dung-shard-01-<RUN_ID> --workers 5
```
...
- Người phụ trách **Dũng**, shard **01**, RUN_ID, Git HEAD và thay đổi local ảnh hưởng code.
