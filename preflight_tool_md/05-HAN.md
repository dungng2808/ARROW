# Giao việc preflight — Hán — shard 05
...
- Có `shards-5/shard-05.json`, `shards-5/summary.json`, `shards-5/README.md`, `preflight.md`, `preflight_tool/TEST_REPORT_FIX_FOUR_20260923.md`, `Java-version/AGENTS.md`. File phân công parse được và hash khớp summary.
...
Agent hãy đọc **toàn bộ file này**, các AGENTS.md áp dụng và tài liệu được dẫn dưới đây, rồi thực hiện trên máy của **Hán**. Không chỉ đưa lại hướng dẫn hoặc dừng ở index-only: mục tiêu là chạy hết phần được giao, kiểm tra output và viết báo cáo bàn giao nếu môi trường đáp ứng điều kiện an toàn.

- Repository: ARROW trên máy hiện tại; tự tìm đường dẫn, không hard-code username/ổ đĩa của người khác.
- File phân công duy nhất: `shards-5/shard-05.json`.
- Số class cần xử lý: **17163**; **1.882 repository**.
...
3. Xác nhận CLI có `--shard`. Kiểm SHA-256 file `shard-05.json` với entry tương ứng trong `shards-5/summary.json`; số class phải đúng 17163. Nếu thiếu/mismatch thì dừng trước build và báo lỗi.
...
```text
python -m preflight.cli --config ../Java-version/config.local.toml --input-root ../classes2test --shard ../shards-5/shard-05.json --index-only --output-dir runs/han-shard-05-<RUN_ID>-check
```

Phải kiểm tra:

- Exit code 0 và `provenance.json` có `shard_id = shard-05`, `classes_selected = 17163`.
- `manifests/class_candidates.jsonl` có đúng 17163 task ID duy nhất và đúng tập ID trong shard.
...
```text
python -m preflight.cli --config ../Java-version/config.local.toml --input-root ../classes2test --shard ../shards-5/shard-05.json --workers 5 --output-dir runs/han-shard-05-<RUN_ID>
```

Nếu cần dừng, nhấn `Ctrl+C` một lần và chờ tool thoát mã `130`. Session sau tiếp tục bằng:

```text
python -m preflight.cli --resume --output-dir runs/han-shard-05-<RUN_ID> --workers 5
```
...
1. Full run exit code 0; `summary.total = 17163`.
2. JSONL có đúng 17163 kết quả và task ID duy nhất; tập task ID bằng **chính xác** tập trong shard, không thiếu hoặc thừa. CSV có cùng tập ID.
3. Tổng `summary.by_status` bằng 17163; từng số lượng khớp JSONL.
...
- Người phụ trách **Hán**, shard **05**, RUN_ID, Git HEAD và thay đổi local ảnh hưởng code.
...
- Expected **17163**, actual total, số missing/extra/duplicate và kết quả từng phép đối soát mục 6.
...
Cuối cùng trả lời người dùng bằng tiếng Việt: đã xử lý bao nhiêu/17163 class, phân bố trạng thái chính, kết quả đối soát, đường dẫn HANDOFF và phần còn chặn. Nếu mọi điều kiện đạt, kết thúc với COMPLETED_DISCOVERY.
