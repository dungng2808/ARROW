...
def test_cli_writes_all_contract_outputs(dataset_factory, tmp_path, monkeypatch):
    ...
    def simulated_run(candidates, config, checkpoint=None, session_id=None, on_checkpoint=None):
        ...
        checkpoint.complete_task(candidate.task_id, attempt, result.to_dict())
        if on_checkpoint:
            on_checkpoint()
        return [result]
