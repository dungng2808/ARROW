...
- Trên Windows, kiểm kê mọi ổ FileSystem theo
  `preflight_tool_md/00-PREPARE-WINDOWS.md` trước khi chọn artifact. Kiểm PATH,
  registry, vị trí portable và tìm có mục tiêu `java.exe`/`javac.exe` trên từng
  ổ. Ghi ổ không truy cập được. Không coi “không nằm trong PATH” là “chưa cài”.
...
1. Kiểm mọi JDK đã tìm thấy trên các ổ, kể cả ngoài `Java-version/runtime/`.
   Chỉ tái sử dụng JAVA_HOME có `java` và `javac` chạy exit code 0, đúng
   OS/CPU/major và phiên bản đã ghi. Không tái sử dụng binary của OS/CPU khác
   chỉ vì tên folder trùng. Nếu có nhiều bản hợp lệ, chọn bản phù hợp với
   phiên bản nhóm đã thống nhất và ghi rõ lý do. Không copy/di chuyển bản đã có
   chỉ để ép nó vào `runtime/`.
2. Với major vẫn thiếu sau kiểm kê, tải archive vào `Java-version/.cache/`;
   xử lý lỗi mạng rõ ràng và kiểm tra SHA-256 trước khi giải nén hoặc thực thi
   binary.
...
4. Xác định JAVA_HOME thực tế: thư mục chứa `bin/java` và `bin/javac` (Windows
   ...
   không thêm tầng platform hoặc folder tên archive **đối với JDK mới cài**.
   JDK đã tái sử dụng ở ổ khác giữ nguyên đường dẫn. JAVA_HOME trên macOS có
...
Cấu trúc JDK **mới cài** trên cả macOS và Windows:
...
Khi setup JDK **độc lập** và đã có dependency preflight, có thể kiểm input từ
root ARROW bằng:
...
Nếu agent đồng thời thực hiện `preflight_tool_md/00-PREPARE-WINDOWS.md`, dùng
chính output index-only của file đó; không lập chỉ mục thêm lần nữa chỉ để
kiểm JDK.
